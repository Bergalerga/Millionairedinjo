;(function ($, window, document, undefined) {
  ...
  Plugin.prototype = {
    ...
    start: function () {
      ...
      this.interval = setInterval(jQuery.proxy(this._draw, this), 500);
    },
    ...
    _draw: function () {
      var secondsElapsed = (new Date().getTime() - this.startedAt.getTime())/1000,
          endAngle = (Math.PI*1.5) + (((Math.PI*2)/this.settings.seconds) * secondsElapsed);
      this._clearRect();
      this._drawCountdownShape(Math.PI*3.5, false);
      if (secondsElapsed < this.settings.seconds) {
        this._drawCountdownShape(endAngle, true);
        this._drawCountdownLabel(Math.round(secondsElapsed));
      } else {
